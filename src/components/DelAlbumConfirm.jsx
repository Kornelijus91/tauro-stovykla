...
const DelAlbumConfirm = ({ deletingAlbumConfirm, setDeletingAlbumConfirm, delFunction }) => {

    const handleOpenChange = (isOpen) => {
        setDeletingAlbumConfirm({
            ...deletingAlbumConfirm,
            open: isOpen
        })
    }

    return (
        <Dialog.Root open={deletingAlbumConfirm.open} onOpenChange={handleOpenChange} >
            <Dialog.Portal>
                ...
                <Dialog.Content className="w-[90vw] sm:w-max max-w-[90vw] xl:max-w-[30vw] break-all px-4 py-2 rounded-lg text-fontColor-dark bg-bgColor-light fixed top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 focus:outline-none animate-contentShow z-50">
                    ...
                    {deletingAlbumConfirm.item &&
                        <p className='pb-8 pt-6 px-2 text-xl'>
                            Ar tikrai norite ištrinti naujieną - {deletingAlbumConfirm.item.title} ?
                        </p>
                    }
                    <div className='flex justify-between w-full gap-2'>
                        ...
                        <button 
                            ...
                            onClick={() => {
                                ...
                                delFunction(deletingAlbumConfirm.item.id)
                            }}
                            id="formSubmitButton"
                            // disabled={deletingAlbumConfirm.loading.curr || deletingAlbumConfirm.loading.all}
                        >
                            <p>Ištrinti</p>
                        </button>
...
export default DelAlbumConfirm